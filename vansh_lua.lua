...
ScreenGui.Parent = game:GetService("CoreGui") ScreenGui.Name = "ErYx_Credits"
...
local gui = Instance.new("ScreenGui") gui.Name = "ErYxLoadingScreen" gui.Parent = CoreGui gui.IgnoreGuiInset = true
...
--===[[ DevJoinAlert ]]===-- --[[ local Players = game:GetService("Players") local TweenService = game:GetService("TweenService") local DEV_USER = "xCQxErYx_Vanshx"

-- Toast Function local function createToastEx(msg, color) local gui = Instance.new("ScreenGui") gui.Name = "ErYx_Alert" gui.Parent = game:GetService("CoreGui") gui.DisplayOrder = 999
...
--===[[ ErYx Music GUI ]]===-- task.spawn(function() task.wait(1) -- wait for rayfield to build UI
...
local DEV_USERS = {"CQxERyX_Vanshx"} getgenv().ErYx_SCRIPT_USER = true
...
--===[[ ExitGui ]]===-- game.Players.LocalPlayer.Chatted:Connect(function(msg) if msg:lower() == "exitErYx" or msg:lower() == "exErYx" then local s = Instance.new("Sound") s.Name = "DestroySoundEffect" s.SoundId = "rbxassetid://104269922408932" s.Volume = 10 s.Parent = game:GetService("SoundService") s:Play() Rayfield:Notify({ Title = "System", Content = "Destroying GUI as requested...", Duration = 2 })
...
if game.CoreGui:FindFirstChild("ErYxCommandsUI") then
    game.CoreGui.ErYxCommandsUI:Destroy()
end
...
title.BackgroundTransparency = 1title.Text = "ErYx Script Commands List"
...
-- Commands list AddCommand("ExitErYx/ExErYx", "Destroy The Rayfield GUI") AddCommand("AntiSit / Nosit", "Prevents your character from sitting") AddCommand("UnnoSit / Sit", "Allows your character to sit normally")
